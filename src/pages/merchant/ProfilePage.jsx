      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-sm font-bold text-[#111827]">{phone || 'Não cadastrado'}</span>
            <Badge
              verified={verified}
...
function ShortcutsCard({ userData }) {
  const storeSlug = userData?.storeSlug || userData?.slug || null

  const shortcuts = [
    {
      label: 'Configurações da loja',
      description: 'Horários, Pix, endereço',
    ...
    storeSlug
      ? {
          ...
          description: `pratoby.com/${storeSlug}`,
          href: `/${storeSlug}`,
          icon: FiExternalLink,
          external: true,
        }
  ...
  return (
    <SectionCard icon={FiChevronRight} title="Atalhos" description="Acesso rápido">
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {shortcuts.map(item => {
          const Icon = item.icon
          const inner = (
            <div className="flex items-center gap-3 rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3 transition hover:border-orange-100 hover:bg-orange-50">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-2xl bg-white text-[#f97316] shadow-sm ring-1 ring-gray-100">
                <Icon size={16} />
              </span>
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-black text-[#111827]">{item.label}</p>
                <p className="truncate text-[11px] font-semibold text-[#9ca3af]">{item.description}</p>
              </div>
              <FiChevronRight size={15} className="shrink-0 text-gray-400" />
            </div>
          )
          if (item.external) {
            return (
  ...
  return (
    <main className="min-h-screen bg-[#f9fafb] pb-20 text-[#111827]">
      ...
      <div className="mx-auto max-w-4xl space-y-4 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <ShortcutsCard userData={userData} />
